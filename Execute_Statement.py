...
class ExecuteStatement:
    ...
    def execute_main(self, data_dict, question):
        ...
        if question_types == 'character_to_simpleIssue':
            ...
            answer_list.extend(
                self.execute_question(question_types, labels_dict))  # 通过标签和类型来找到cql语句从而得到问题的答案
        elif question_types == 'character_to_problemtoschool':
            answer_list.extend(
                self.execute_question(question_types, labels_dict))  # 通过标签和类型来找到cql语句从而得到问题的答案
        elif question_types == 'character_to_book':
            answer_list.extend(
                self.execute_question(question_types, labels_dict))  # 通过标签和类型来找到cql语句从而得到问题的答案
        elif question_types == 'samePartner':
            answer_list.extend(
                self.execute_question(question_types, labels_dict))  # 通过标签和类型来找到cql语句从而得到问题的答案
    ...
    def execute_question(self, question_type, nodes):
        ...
        if question_type == 'character_to_simpleIssue':
            ...
            for i in nodes.keys():
                if 'character' == i:
                    for i in nodes['character']:
                        my_input.append(i)
                    flag=1
                    break
                elif 'school' == i:
                    for i in nodes['school']:
                        my_input.append(i)
                    flag=1
                    break
                elif 'literaryWorks' == i:
                    for i in nodes['literaryWorks']:
                        my_input.append(i)
                    flag=1
                    break
            ...
            if len(re_list) == 1:
                print("1")
                if '作者' in self.question:
                    cql = ["match (n)-[r:`文学作品`]-(m) where m.name='{0}' return n.name".format(my_input[0])]
                    for j in cql:
            ...
            if len(re_list) == 2:
                ...
                if re.search(r".*的.*和.*",self.question):
                    for i in re_list:
                        cql = [
                        ...
                        for j in cql:
                            answer_list.extend([i["m.name"] for i in self.connect_graph.run(j) if i["m.name"] != ''])
                    print('成功9')
                if re.search(r".*的.*的.*", self.question):
                    cql = [
        ...
        #从这里开始写
        ...
        elif question_type == 'character_to_problemtoschool':
            ...
            if 'filmAndTelevisionWork' in list_processed:
                filmAndTelevisionWork = nodes['filmAndTelevisionWork'][0]
                my_input.append(filmAndTelevisionWork)
            if 'character' in list_processed:
                character = nodes['character'][0]
                my_input.append(character)
            if my_input == []:
                return answer_list
            for i in relationship:
            ...
            re_list = self.myfind_max(re_list)
            re_list = self.relist_sort(re_list, self.question)
            my_input = self.myfind_max(my_input)
            my_input=self.node_sort(my_input,self.question)
            my_input=list(set(my_input))
            if '演员' in self.question and '主要演员' not in re_list:
                re_list.append('主要演员')
            if '毕业院校' in re_list:
                re_list.remove('毕业院校')
            ...
            if len(my_input)==1:
                cql = [
                    "match (n)-[r:`{0}`]->(m)-[q:`毕业院校`]->(g) where n.name='{1}' and g.name='{2}' return m.name".format(
                        re_list[0], my_input[0], my_input[0])]
                print(cql)
                for j in cql:
                    answer_list.extend([i["m.name"] for i in self.connect_graph.run(j) if i["m.name"] != ''])
                    print([i["m.name"] for i in self.connect_graph.run(j) if i["m.name"] != ''])
                if '多少' in self.question:
                    return str(len(answer_list)).split(',')
                else:
                    return answer_list
            else:
                cql = [
                    "match (n)-[r:`{0}`]->(m)-[q:`毕业院校`]->(g) where n.name='{1}' and g.name='{2}' return m.name".format(
                        re_list[0], my_input[0], my_input[1])]
                print(cql)
                for j in cql:
                    answer_list.extend([i["m.name"] for i in self.connect_graph.run(j) if i["m.name"] != ''])
                    print([i["m.name"] for i in self.connect_graph.run(j) if i["m.name"] != ''])
                if '多少' in self.question:
                    ...
                    return answer_list
        elif question_type == 'character_to_book':
            print('character_to_book')
            character=[]
            re_list = []
            list_processed = nodes.keys()
            if 'character'  in list_processed:
                character.append(nodes['character'][0])
            if character == []:
                return answer_list
            else:
                if '书' in self.question or '诗' in self.question:
                    cql = ["match (n)-[r:`文学作品`]->(m) where n.name='{0}' return m.name".format(character[0])]
                    for j in cql:
                        answer_list.extend([i["m.name"] for i in self.connect_graph.run(j) if i["m.name"] != ''])
                        print([i["m.name"] for i in self.connect_graph.run(j) if i["m.name"] != ''])
                    return answer_list
                else:
                    re_list=["主要作品","文学作品","代表作品","摄影作品","音乐作品"]
                    for i in re_list:
                        cql = ["match (n)-[r:`{0}`]->(m) where n.name='{1}' return m.name".format(i,character[0])]
                        for j in cql:
                            answer_list.extend([i["m.name"] for i in self.connect_graph.run(j) if i["m.name"] != ''])
                        ...
                        return answer_list
        #张荐写过哪些作品？
        elif question_type == "samePartner":
            same_character = set()
            cql = [r"match (m:`人物`)-[r:`搭档`]->(n) where m.name='{0}' return n.name".format(i) for i in
                   nodes["character"]]
            for j in cql:
                temp_set = {i["n.name"] for i in self.connect_graph.run(j)}
                if not same_character:
                    same_character = temp_set
                else:
                    same_character.intersection(temp_set)
            if "有没有" in self.question:
                if same_character:
                    return ['有']
                else:
                    return ['没有']
                    # answer_list.extend([i["m.name"] for i in self.connect_graph.run() if i["m.name"] != ''])
            else:  # 包含"有哪些"
                return list(same_character)
            # 唐雅菁和黄莺有哪些共同的搭档？
